...
def _build_ground_truth(body: OmniVerifierRunRequest) -> str:
    ...
    if not body.verifier or answer is None:
        raise ValueError("omni_verifier requires either ground_truth or verifier plus expected_answer/answer")
...
class OmniVerifierResourcesServer(SimpleResourcesServer):
    ...
    async def verify(self, body: OmniVerifierVerifyRequest) -> OmniVerifierVerifyResponse:
        ...
        return OmniVerifierVerifyResponse(
            **body.model_dump(exclude={"ground_truth", "verifier", "expected_answer", "answer"}),
            reward=float(reward),
            is_correct=bool(is_correct),
            ground_truth=ground_truth,
            verifier=body.verifier,
            expected_answer=(body.expected_answer if body.expected_answer is not None else body.answer),
        )
