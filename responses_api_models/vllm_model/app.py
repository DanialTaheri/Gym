        ...
        if key == "required_prompt_token_ids":
            if isinstance(value, list):
                summary[key] = {
                    ...
                    "first8": value[:8],
                    "last8": value[-8:],
                    "checksum": sum((idx + 1) * int(token_id) for idx, token_id in enumerate(value)) % 1_000_000_007,
                }
            else:
...
class VLLMModel(SimpleResponsesAPIModel):
    ...
    async def _get_prompt_token_ids(self, client: NeMoGymAsyncOpenAI, body_dict: Dict[str, Any]) -> List[int]:
        tokenize_response = await self._get_tokenize_response(client, body_dict)
        return tokenize_response["tokens"]

    async def _get_tokenize_response(self, client: NeMoGymAsyncOpenAI, body_dict: Dict[str, Any]) -> Dict[str, Any]:
        tokenize_response = await client.create_tokenize(**self._get_tokenize_body_dict(body_dict))
    ...
    async def chat_completions(
        ...
        if vllm_max_model_len is not None:
            max_input_tokens = (
                vllm_max_model_len if max_input_tokens is None else min(max_input_tokens, vllm_max_model_len)
            )
        ...
        if max_input_tokens is not None and prompt_token_ids is not None:
            ...
            body_dict["max_tokens"] = (
                remaining_budget if requested_max_tokens is None else min(requested_max_tokens, remaining_budget)
            )
        ...
        if self.config.return_token_id_information:
            ...
            if direct_generation_token_ids is not None:
                ...
                try:
                    logprob_token_ids = [_token_id_from_logprob_token(log_prob["token"]) for log_prob in log_probs]
                except ValueError:
                    logprob_token_ids = None
                ...
                generation_token_ids = [_token_id_from_logprob_token(log_prob["token"]) for log_prob in log_probs]
            ...
            if debug_prefill_logprobs or refit_generation_logprobs:
                ...
                if prompt_token_ids and generation_token_ids:
                    ...
                    if max_context is not None and len(sequence_token_ids) >= int(max_context):
                        debug_status = f"skipped_context_full:{len(sequence_token_ids)}>={int(max_context)}"
                    else:
                        ...
                        try:
                            rescore_dict = await client.create_chat_completion(**rescore_body_dict)
                            prefill_log_probs, debug_status = _extract_prompt_logprobs_for_tokens(
                            ...
                            if debug_status == "ok":
                                ...
                                diffs = [
                                    ...
                                    for decode_lp, prefill_lp in zip(generation_log_probs, prefill_log_probs)
                                ]
                ...
                debug_prefill_logprob_info["debug_vllm_prefill_generation_logprob_status"] = debug_status
            ...
            if refit_generation_logprobs:
                ...
                elif prefill_log_probs_for_generation is not None and len(prefill_log_probs_for_generation) == len(
                    generation_log_probs
                ):
                    debug_prefill_logprob_info["debug_vllm_decode_generation_log_probs"] = generation_log_probs
                ...
                match part_param["type"]:
                    ...
                    case "input_audio":
                        if "input_audio" in part_param:
                            converted_parts.append({"type": "input_audio", "input_audio": part_param["input_audio"]})
                        else:
                            audio_url = part_param.get("audio_url", part_param.get("audio", ""))
                            if isinstance(audio_url, dict):
                                audio_url = audio_url.get("url", "")
                            converted_parts.append({"type": "audio_url", "audio_url": {"url": audio_url}})
                    case "audio_url":
