...
from nemo_rl.environments.image_zoom_gym_tool import (
    ImageZoomGymToolConfig,
    ImageZoomGymToolLogic,
    has_malformed_image_zoom_tool_markup,
    parse_image_zoom_tool_calls,
)
...
class ImageZoomAgentConfig(BaseResponsesAPIAgentConfig):
...
class ImageZoomAgentRunRequest(BaseRunRequest):
    model_config = ConfigDict(extra="allow")

    image_zoom_base_agent_ref: Optional[dict[str, Any]] = None


class ImageZoomAgentVerifyRequest(BaseVerifyRequest):
    model_config = ConfigDict(extra="allow")


class ImageZoomAgentVerifyResponse(BaseVerifyResponse):
    model_config = ConfigDict(extra="allow")

    base_reward: float = 0.0
    image_zoom_aux_reward: float = 0.0
    image_zoom_tool_call_count: int = 0
    image_zoom_tool_error_count: int = 0
    image_zoom_crop_paths: list[str] = []
    image_zoom_generation_image_paths: list[list[str]] = []
    image_zoom_base_agent_ref: Optional[dict[str, Any]] = None
...
class ImageZoomAgent(SimpleResponsesAPIAgent):
    config: ImageZoomAgentConfig
    _logic: ImageZoomGymToolLogic = PrivateAttr()

    def model_post_init(self, context: Any) -> None:
        super().model_post_init(context)
        self._logic = ImageZoomGymToolLogic(self._logic_config())

    def _logic_config(self) -> ImageZoomGymToolConfig:
        return {
    ...
    def _resource_name(self, body: ImageZoomAgentRunRequest) -> str:
        base_agent_ref = body.image_zoom_base_agent_ref
        if not base_agent_ref:
            base_agent_ref = (body.model_extra or {}).get("image_zoom_base_agent_ref")
        if not base_agent_ref:
            raise ValueError("image_zoom_base_agent_ref is required")
    ...
    async def responses(
        ...
        result, model_cookies, _ = await self._run_image_zoom_loop(
            body=body,
            row_metadata={},
    ...
    async def _run_image_zoom_loop(
        self,
        body: NeMoGymResponseCreateParamsNonStreaming,
        ...
        for _ in range(self.config.max_steps):
            ...
            tool_calls = parse_image_zoom_tool_calls(assistant_text)
            malformed_tool_attempt = has_malformed_image_zoom_tool_markup(
                assistant_text
            )
        ...
        if final_response is None:
            raise RuntimeError("ImageZoomAgent did not receive a model response")

        final_response = final_response.model_copy(
            update={"output": new_outputs, "usage": usage}
        )
        rollout_info = {
            "image_zoom_aux_reward": aux_reward,
            "image_zoom_tool_call_count": int(metadata.get("tool_call_count", 0)),
            "image_zoom_tool_error_count": int(metadata.get("tool_error_count", 0)),
            "image_zoom_crop_paths": list(metadata.get("crop_paths", [])),
            "image_zoom_generation_image_paths": generation_image_paths,
        }
        return final_response, model_cookies, rollout_info

    async def run(self, request: Request, body: ImageZoomAgentRunRequest) -> ImageZoomAgentVerifyResponse:
        ...
        model_response, model_cookies, rollout_info = await self._run_image_zoom_loop(
        ...
        verify_request = ImageZoomAgentVerifyRequest.model_validate(verify_request_body)
        ...
        aux_reward = float(rollout_info["image_zoom_aux_reward"])
        verify_response_json["base_reward"] = base_reward
        verify_response_json["image_zoom_aux_reward"] = aux_reward
        verify_response_json["reward"] = base_reward + aux_reward
        verify_response_json["image_zoom_base_agent_ref"] = body.image_zoom_base_agent_ref
        verify_response_json.update(rollout_info)
        verify_response_json["response"] = model_response.model_dump()
        return ImageZoomAgentVerifyResponse.model_validate(verify_response_json)
