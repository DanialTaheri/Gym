...
from responses_api_agents.image_zoom_agent.app import (
    ImageZoomAgent,
    ImageZoomAgentConfig,
)
...
def test_image_zoom_tool_markup_validation_allows_clean_xml() -> None:
    text = (
    ...
    assert not has_malformed_image_zoom_tool_markup(text)


def test_image_zoom_tool_markup_validation_rejects_nested_or_extra_tags() -> None:
    text = (
        "<tool_call><function=image_zoom_in_tool>"
        "<parameter=bbox_2d>[0, 0, 500, 500]</parameter>"
        "<parameter=label>bad\n</think>\n<tool_call>"
    ...
    assert has_malformed_image_zoom_tool_markup(text)


def test_image_zoom_agent_runs_tool_loop_and_delegates_reward(tmp_path: Path) -> None:
    ...
    async def _post_side_effect(*, server_name: str, url_path: str, **kwargs: Any):
        ...
        if server_name == "policy_model" and url_path == "/v1/responses":
            ...
            if call_index == 2:
                ...
                assert "include_stop_str_in_output" in request.metadata["extra_body"]
                return _FakeClientResponse(
                    _assistant_response(
                        response_id="tool",
                        text=tool_text,
    ...
    agent = ImageZoomAgent(config=config, server_client=server_client)
    client = TestClient(agent.setup_webserver())

    response = client.post(
        "/run",
        json={
            "image_zoom_base_agent_ref": {
    ...
    assert payload["image_zoom_aux_reward"] == 0.02
    assert payload["reward"] == 1.02
    assert payload["image_zoom_tool_call_count"] == 1
    assert payload["image_zoom_tool_error_count"] == 0
    assert len(payload["image_zoom_crop_paths"]) == 1
    assert Path(payload["image_zoom_crop_paths"][0]).exists()
    assert payload["image_zoom_generation_image_paths"][0] == []
    assert payload["image_zoom_generation_image_paths"][1] == payload["image_zoom_crop_paths"]
